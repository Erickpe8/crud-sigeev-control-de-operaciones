...
class CentralPanelController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        /**
         * Definición de módulos disponibles (catálogo completo).
         * - permission: string de Spatie (opcional). Si está vacío, no filtra por permiso.
         * - roles: arreglo de roles habilitados para ver el módulo.
         * - route: nombre de ruta Laravel (debe existir).
         */
        $modules = [
            // SUPERADMIN
            [
        ...
        // Función ayuda: valida rol y permiso
        $canSee = function (array $module) use ($user) {
        ...
        // Filtrado final de módulos visibles para el usuario
        $availableModules = array_values(array_filter($modules, $canSee));

        // Sidebar: puedes reutilizar módulos o definir items separados
        $sidebarItems = $this->buildSidebar($user);
    ...
    /**
     * Construye los items del sidebar según rol/permisos.
     * Retorna arreglo de: ['route' => string, 'label' => string, 'icon' => svg]
     */
    protected function buildSidebar($user): array
    {
        $items = [
            [
                'route' => 'panel', // ruta al propio panel
                'label' => 'Panel',
            ...
            // Agrega más items según tu módulo/permiso...
        ];

        // Filtrar por rol + permiso
        $items = array_values(array_filter($items, function ($item) use ($user) {
            $rolesOk = empty($item['roles']) || $user->hasAnyRole($item['roles']);
            $permOk  = empty($item['permission']) || $user->can($item['permission']) || $user->hasPermissionTo($item['permission']);
            return $rolesOk && $permOk;
        }));
