...
<form id="adminCreateForm" class="w-[720px] bg-white rounded-lg shadow-lg p-6 relative">
    ...
    <div class="flex gap-4 mt-6">
        ...
        <a href="{{ $dashUrl }}" class="flex items-center justify-center gap-2 bg-gray-600 text-white px-6 py-2 rounded w-full">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5" viewBox="0 0 24 24" fill="currentColor">
...
<script>
    ...
    async function crearUsuario() {
        ...
        // Solo superadmin verá el select de rol
        const roleEl = document.getElementById('role');
        if (roleEl && roleEl.value) data.role = roleEl.value;

        try {
            await axios.get('/sanctum/csrf-cookie'); // si usas Sanctum
